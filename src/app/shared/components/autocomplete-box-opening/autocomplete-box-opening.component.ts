...
export class AutocompleteBoxOpeningComponent {
    ...
    async initValue() {
        ...
        if (value) {
            const users = await this.databaseStorage.getData<BoxOpening>(NameModuleDatabase.BoxOpenings);
    ...
    private async filteredUsers(search: string): Promise<void> {
        this.loading = true;
        this.box_openings = (
            await this.databaseStorage.getData<BoxOpening>(NameModuleDatabase.BoxOpenings)
        ).filter(
            (u) =>
